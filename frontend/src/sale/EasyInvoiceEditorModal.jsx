...
const buildMissingText = (missingFields = []) => (
    missingFields.length ? `Vui lòng nhập đủ: ${missingFields.join(', ')}.` : ''
);
...
export default function EasyInvoiceEditorModal({
    ...
    return (
        <div
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 1760, background: 'rgba(15,23,42,.58)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: compact ? 0 : 18 }}
        >
            <div
                onClick={(event) => event.stopPropagation()}
                style={{ width: '100%', maxWidth: 1360, height: compact ? '100dvh' : 'min(96vh, 980px)', borderRadius: compact ? 0 : 28, background: 'linear-gradient(180deg, rgba(255,255,255,.98), rgba(248,250,252,.98))', boxShadow: '0 28px 64px rgba(15,23,42,.28)', border: '1px solid rgba(15,23,42,.08)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
            >
                ...
                {statusText ? (
                    <div style={{ padding: '10px 18px', borderBottom: '1px solid rgba(15,23,42,.06)', background: errorText ? '#fef2f2' : publishDisabled ? '#fff7ed' : '#eff6ff', color: errorText ? '#b91c1c' : publishDisabled ? '#9a3412' : '#1d4ed8', fontSize: 12, lineHeight: 1.6, fontWeight: 700 }}>
                        {statusText}
                    </div>
                ) : null}
