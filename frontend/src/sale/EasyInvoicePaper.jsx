...
const words1k = (num) => {
    const d = ['khong', 'mot', 'hai', 'ba', 'bon', 'nam', 'sau', 'bay', 'tam', 'chin'];
    const h = Math.floor(num / 100);
    const t = Math.floor((num % 100) / 10);
    const u = num % 10;
    const parts = [];
    if (h > 0) parts.push(d[h], 'tram');
    if (t > 1) {
        parts.push(d[t], 'muoi');
        if (u === 1) parts.push('mot');
        else if (u === 5) parts.push('lam');
        else if (u > 0) parts.push(d[u]);
        return parts.join(' ');
    }
    if (t === 1) {
        parts.push('muoi');
        if (u === 5) parts.push('lam');
        else if (u > 0) parts.push(d[u]);
        return parts.join(' ');
    }
    if (u > 0) {
        if (h > 0) parts.push('linh');
        parts.push(u === 5 && h > 0 ? 'nam' : d[u]);
    }
    return parts.join(' ');
};
const moneyToWords = (value) => {
    let amount = Math.round(Math.abs(Number(value || 0)));
    if (!amount) return 'Khong dong.';
    ...
    return `${text.charAt(0).toUpperCase()}${text.slice(1)} dong.`;
};
...
const UI = {
    ...
    cellLabel: { display: 'block', fontSize: 10, color: '#7c2d12', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.3, fontWeight: 700 },
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : '160px minmax(0,1fr) 220px', gap: 14, alignItems: 'start' }}>
                        <div style={{ minHeight: 106, borderRadius: 18, border: '2px double rgba(120,53,15,.5)', background: 'rgba(255,255,255,.65)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: '#7c2d12', padding: 12 }}>
                            <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: 1.2 }}>VAN KIM</div>
