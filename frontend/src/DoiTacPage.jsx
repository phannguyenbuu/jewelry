...
const EMPTY_FORM = {
    ...
    loai_khach: 'Nhà cung cấp', nguon: '', sao: 0, ghi_chu: '', ngay_them: today()
};
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
                {[
                    { label: 'Tổng đối tác', value: stats.total, icon: '🤝', color: '#6366f1', bg: '#f5f3ff', border: '#ddd6fe' },
                    { label: 'Đánh giá TB', value: stats.avg_sao + ' ★', icon: '⭐', color: '#f59e0b', bg: '#fffbeb', border: '#fde68a' },
                    { label: 'Đối tác 5 sao', value: stats.c5, icon: '🌟', color: '#16a34a', bg: '#f0fdf4', border: '#bbf7d0' },
                    { label: 'Đối tác ≥ 4 sao', value: stats.c4, icon: '💎', color: '#0ea5e9', bg: '#f0f9ff', border: '#bae6fd' },
                ].map(s => (
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }} />
                <button onClick={openAdd} style={{ ...btn('#0ea5e9') }}>+ Thêm đối tác</button>
            </div>
            ...
            {filtered.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8', background: 'white', borderRadius: 14, border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>{search || filterSao ? 'Không tìm thấy đối tác' : 'Chưa có đối tác'}</div>
                    <div style={{ fontSize: 13 }}>Nhấn "+ Thêm đối tác" để bắt đầu</div>
                </div>
            ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 14 }}>
                    {filtered.map(kh => (
                        ...
                        >
                            ...
                            {kh.ghi_chu && (
                                <div style={{ fontSize: 12, color: '#475569', background: '#f8fafc', borderRadius: 8, padding: '8px 10px', borderLeft: '3px solid #0ea5e9', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                                    📝 {kh.ghi_chu}
                                </div>
                            )}
                            {kh.dia_chi && (
            ...
            {/* Add/Edit Modal */}
            <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'add' ? '🤝 Thêm đối tác mới' : `✏️ Sửa — ${modal?.ten}`} maxWidth={640}>
                <form onSubmit={save}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                        ...
                        <Field label="Loại đối tác">
                            <select style={inp} value={form.loai_khach} onChange={e => setForm({ ...form, loai_khach: e.target.value })}>
                                {['Nhà cung cấp', 'Đại lý', 'Xưởng gia công', 'Vận chuyển', 'Ngân hàng', 'Khác'].map(l => <option key={l}>{l}</option>)}
                            </select>
                        </Field>
                    ...
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
                        ...
                        <button type="submit" style={{ ...btn('#0ea5e9') }}>{modal === 'add' ? '+ Thêm đối tác' : 'Lưu thay đổi'}</button>
                    </div>
                </form>
            </Modal>
            ...
            {/* Detail Modal */}
            {detailModal && (
                <Modal open={true} onClose={() => setDetailModal(null)} title="🤝 Chi tiết đối tác" maxWidth={560}>
            ...
            {confirmDel && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: 'white', borderRadius: 14, padding: 28, maxWidth: 340, textAlign: 'center' }}>
                        ...
                        <div style={{ fontWeight: 700, marginBottom: 8, fontSize: 15 }}>Xóa đối tác này?</div>
